...
Current phase: Wave 0 bootstrap and Wave 1 contract handoff are closed through W1-S7/C1-K. Wave 1.5 contract CI readiness is accepted through CI15-A/CI15-B/CI15-C/CI15-D/CI15-E/CI15-F with no recorded CI-debt exception. Wave 2 is closed through K2-A/K2-B/K2-C/K2-D/K2-E/K2-F/K2-G/K2-H and the Wave 2 Meta closeout gate: the no-LLM Aster seed run is deterministic and writes smoke-validated manifest/snapshot/event/state-diff artifacts. Wave 3 B3-A/B3-B is accepted with the first stdlib-only Python brain CLI skeleton and model-policy loader; B3-C/B3-D/B3-F is accepted with deterministic mock packet output, strict schema validation, and L1 pulse prompt-context boundaries; B3-E/B3-G is accepted with shell-only OpenRouter env validation and deterministic dev/mock cognition trace + cost artifacts; B3-H is accepted with Track E packet/trace/cost artifact smoke validation. The active frontier is now the Wave 3 Meta closeout gate. This repository is not ready for public use. The detailed design canon, world bible, implementation plan, and private coordination notes are local/private until an explicit export review approves public release material.
...
| `src/` | Source code, including the Wave 1 contract layout/schema drafts, Wave 2 C# deterministic core/headless runner, and Wave 3 Python brain skeleton. |
...
| `tools/` | Developer utilities, including the Wave 1 contract schema checker and Wave 2 core artifact smoke validator. |
| `infra/` | Optional deployment/remote-run material later. Empty in this skeleton. |

## Current Non-Goals

- no LLM-driven or multi-step simulation beyond the Wave 2 deterministic Aster T0 seed slice yet
- no deterministic runtime beyond the Wave 2 C#/.NET headless core artifact slice yet
- no Python brain provider/runtime beyond Wave 3 local mock packet output, schema validation, prompt-context draft surfaces, shell-only OpenRouter env validation, and dev/mock trace/cost artifacts yet
- no production runtime schema service or generated contract bindings; current validation tooling is developer-only
